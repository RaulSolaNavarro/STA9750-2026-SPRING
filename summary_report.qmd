...
The adjusted predicted resolution times rank as follows: mobile (1.65 hours), online (2.64 hours), other (3.11 hours), and phone (3.61 hours). The mobile channel has the shortest average resolution time, and the ordering holds before and after controls. However, the channel effect is weak in absolute terms: in the regression model, agency identity and complaint category together account for the large majority of explained variance in resolution time, while submission channel adds only a marginal increment. The adjusted R-squared for a model with only agency and complaint category is substantially higher than one that adds channel, confirming that knowing how a request was filed tells you little once you know what was filed and who handles it.
...
The four analyses point to the same structural finding: **complaint type is the primary organizer of resolution time.** It determines which agency handles a request, how long resolution takes by physical necessity, and whether a published SLA target even exists. Income, weather, and submission channel each show raw associations with resolution time, but those associations shrink substantially after controlling for what type of complaint was filed. The question is not whether complaint type matters, but why certain neighborhoods, weather conditions, and channels are disproportionately associated with the complaint types that take longest to resolve.
